...
namespace SLNStokTakip.Fatura
{
    public partial class frmFaturaKes : Form
    {
        ...
        private void frmFaturaKes_Load(object sender, EventArgs e)
        {
            Temizle();
            SaatBul();
            if (AnaSayfa.Aktarma > 0 && AnaSayfa.Aktarma1 < 0) //formların çakışmaması için anasayfaya aktarma1 tanımladık.
            {
                ListeleIlk();
            }
            else
        ...
        void ListeleIlk() //ürün çıkış için
        {
            ...
            foreach (var k in srg)
            {
                ...
                Liste.Columns[1].Visible = true;   //çıkış kodu ve firma no görünürlükleri değiştirildi
        ...
        void ListeleIki()  //faturakes için
        {
            fkayNo = AnaSayfa.Aktarma1;
            Liste.Rows.Clear();
            int i = 0;
            var srg = (from s in _db.vwFaturaKes   //dbde view oluşturduk ve buraya çektik (tüm verileri bir arada görmek için)
            ...
            {
                Liste.Rows.Add();
                Liste.Rows[i].Cells[0].Value = -1; //viewde id almadık yerine değer atadık
        ...
        public static string yaziyacevir(decimal tutar)
        {
            ...
            string lira = sTutar.Substring(0, sTutar.IndexOf(',')); //tutarın tam kısmı, virgüle kadarki kısmı alacak
            string kurus = sTutar.Substring(sTutar.IndexOf(',') + 1, 2); //virgülden sonrasını alacak
            string yazi = "";

            string[] birler = { "", "BİR", "İKİ", "ÜÇ", "DÖRT", "BEŞ", "ALTI", "YEDİ", "SEKİZ", "DOKUZ" };
            string[] onlar = { "", "ON", "YİRMİ", "OTUZ", "KIRK", "ELLİ", "ALTMIŞ", "YETMİŞ", "SEKSEN", "DOKSAN" };
            string[] binler = { "KATRİLYON", "TRİLYON", "MİLYAR", "MİLYON", "BİN", "" };
            int grupSayisi = 6; //sayıdaki 3lü grup sayısıdır. katrilyon için 6. (1.234,00 daki grup sayısı 2dir.)
            //katrilyonun başına eklenen her değer için grup sayısı arttırılır.
            ...
            for (int i = 0; i < grupSayisi * 3; i += 3)
            {
                ...
                if (grupDegeri == "BİRYÜZ")//biryüz ifadesini düzeltiyor
                    grupDegeri = "YÜZ";
                grupDegeri += onlar[Convert.ToInt32(lira.Substring(i + 1, 1))]; //onlar
                grupDegeri += birler[Convert.ToInt32(lira.Substring(i + 2, 1))]; //birler
                if (grupDegeri != "") //binler
                    grupDegeri += binler[i / 3];
                if (grupDegeri == "BİRBİN") //birbin ifadesini düzeltiyor
                    grupDegeri = "BİN";
                yazi += grupDegeri;
            }
            if (yazi != "")
            ...
            if (kurus.Substring(0, 1) != "0") //kurus olanlar
        ...
        {
            if (!chbManuel.Checked) //chb işaretli değilse
            {
                SaatBul(); //saatbul içindekileri burada yazdığımızda açılış ekranında saat gelmedi,o yüzden metod oluşturduk
            }
            else
            {
                txtSaat.Text = "00:00";
